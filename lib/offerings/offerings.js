...
const createOffering = async (event, context) => {
    // TODO got to add the prices - comes in as an array of price objects.
    // If the object includes an ID then the price already exists, otherwise, create the price and link it to the offering
    // create offering
    /*
        {
...
const fetchOffering = async (event, context) => {
    ...
    try {
        const resp = await fetchInverted(TABLE_NAME, `OFFERING_${offeringId}`, `ACCOUNT_${accountId}`);
