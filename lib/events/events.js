...
const createEvent = async (event, context) => {
    ...
    if (!body.title || !body.description || !body.ticketOptions) { throw new Error('Missing event details') }
    const params = {
        PK: accountId,
        SK: `EVENT_${eventId}`,
        title: body.title,
        ...
        ticketOptions: body.ticketOptions
    };
    try {
...
const updateEvent = async (event, context) => {
    ...
    try {
        ...
        const accountId = event.pathParameters.accountId;
        const eventId = event.pathParameters.eventId;
        const expr = 'SET title = :title, description = :description, ticketOptions = :ticketOptions';
        const values = { ':title': body.title, ':description': body.description, ':ticketOptions': body.ticketOptions }
...
const deleteEvent = async (event, context) => {
    ...
    try {
        const accountId = event.pathParameters.accountId;
        const eventId = event.pathParameters.eventId;
        const resp = await deleteRecord(TABLE_NAME, accountId, `EVENT_${eventId}`);
...
module.exports = { createEvent, fetchEvent, fetchEvents, updateEvent, deleteEvent };